...
import { modeltestTutiral } from './modeltest.model';
...
export const modeltestTutorialExamService = {
  // Create new exam
  async createExam(
    examData: ImodeltestTutorialExam,
  ): Promise<ImodeltestTutorialExam> {
    return await modeltestTutiral.create(examData);
  },
  ...
  async getAllExams(
  ...
  ): Promise<PaginationResult<ImodeltestTutorialExam>> {
    ...
    const [data, total] = await Promise.all([
      modeltestTutiral
        ...
        .limit(limit)
        .exec(),
      modeltestTutiral.countDocuments(query),
    ]);
  ...
  async updateExam(
    id: string,
    updateData: Partial<ImodeltestTutorialExam>,
  ): Promise<ImodeltestTutorialExam | null> {
    return await modeltestTutiral.findByIdAndUpdate(id, updateData, {
      new: true,
    });
  },

  // Delete exam (soft delete)
  async deleteExam(id: string): Promise<ImodeltestTutorialExam | null> {
    return await modeltestTutiral.findByIdAndUpdate(
      id,
      { isDeleted: true },
      { new: true },
    );

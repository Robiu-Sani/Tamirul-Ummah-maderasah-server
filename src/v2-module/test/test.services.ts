...
import { testTutiral } from './test.model';
...
export const testTutorialExamService = {
  // Create new exam
  async createExam(examData: ItestTutorialExam): Promise<ItestTutorialExam> {
    return await testTutiral.create(examData);
  },
  ...
  async getAllExams(
  ...
  ): Promise<PaginationResult<ItestTutorialExam>> {
    ...
    const [data, total] = await Promise.all([
      testTutiral.find(query).sort(sortOptions).skip(skip).limit(limit).exec(),
      testTutiral.countDocuments(query),
    ]);
  ...
  async updateExam(
    id: string,
    updateData: Partial<ItestTutorialExam>,
  ): Promise<ItestTutorialExam | null> {
    return await testTutiral.findByIdAndUpdate(id, updateData, { new: true });
  },

  // Delete exam (soft delete)
  async deleteExam(id: string): Promise<ItestTutorialExam | null> {
    return await testTutiral.findByIdAndUpdate(
      id,
      { isDeleted: true },
      { new: true },
    );

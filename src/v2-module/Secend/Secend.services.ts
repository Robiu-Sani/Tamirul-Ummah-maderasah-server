...
import { SecendTutiral } from './Secend.model';
...
export const SecendTutorialExamService = {
  // Create new exam
  async createExam(
    examData: ISecendTutorialExam,
  ): Promise<ISecendTutorialExam> {
    return await SecendTutiral.create(examData);
  },
  ...
  async getAllExams(
  ...
  ): Promise<PaginationResult<ISecendTutorialExam>> {
    ...
    const [data, total] = await Promise.all([
      SecendTutiral.find(query)
        .sort(sortOptions)
        .skip(skip)
        .limit(limit)
        .exec(),
      SecendTutiral.countDocuments(query),
    ]);
  ...
  async updateExam(
    id: string,
    updateData: Partial<ISecendTutorialExam>,
  ): Promise<ISecendTutorialExam | null> {
    return await SecendTutiral.findByIdAndUpdate(id, updateData, { new: true });
  },

  // Delete exam (soft delete)
  async deleteExam(id: string): Promise<ISecendTutorialExam | null> {
    return await SecendTutiral.findByIdAndUpdate(
      id,
      { isDeleted: true },
      { new: true },
    );
